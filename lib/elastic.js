...
Elastic = {
  ...
  index: function (index, alias, host, port, callback) {
    var client = this.client(host, port);

    if (!client) {
      callback('Can not connect to Elastic Search');
    } else {
      if (typeof alias != 'undefined' && typeof index != 'undefined') {
        client.indices.putAlias({
          ...
          if (error) {
            ...
            if (response.acknowledged) {
              callback(Helper.successMessage('Alias: ' + alias + ' added to index ' + index));
            } else {
              callback(Helper.errorMessage('Could not assign alias'));
            }
          }
        });
      }
